...
public class AssetServiceImpl implements AssetService {
    ...
    public Asset createAsset(Long vId, Long rId, Asset asset) {
        if (asset.getPurchaseCost() <= 0) throw new IllegalArgumentException("Cost > 0");
        if (assetRepo.existsByAssetTag(asset.getAssetTag())) throw new IllegalArgumentException("Duplicate Tag");
        asset.setVendor(vendorRepo.findById(vId).orElseThrow(() -> new ResourceNotFoundException("Vendor")));
        asset.setDepreciationRule(ruleRepo.findById(rId).orElseThrow(() -> new ResourceNotFoundException("Rule")));
    ...
    @Override public Asset getAsset(Long id) { return assetRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Asset")); }
}